...
public class BlockCavernPortal extends BlockPortal
{
	public BlockCavernPortal()
	{
		super();
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		if (CavernMod.proxy.isSinglePlayer() && world.isRemote)
		{
			openRegeneration();
		}
	...
	protected void openMiningRecords()
	{
		FMLClientHandler.instance().showGuiScreen(new GuiMiningRecords());
	}

	@SideOnly(Side.CLIENT)
	protected void openRegeneration()
	{
	...
	public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity)
	{
		...
		WorldData.get(world).addPortal(this, pos);
	...
	public class Size
	{
		...
		protected int calculatePortalHeight()
		{
			...
			outside: for (height = 0; height < 21; ++height)
			{
				for (i = 0; i < width; ++i)
				{
					...
					if (state.getBlock() == BlockCavernPortal.this)
					{
						++portalBlockCount;
					}
		...
		{
			return state.getMaterial() == Material.AIR || state.getBlock() == BlockCavernPortal.this;
		}
