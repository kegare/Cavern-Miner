...
public class GuiMiningPointsEditor extends GuiScreen
{
		...
		if (button.enabled)
		{
			switch (button.id)
			{
				...
				case 4:
					...
					break;
				case 5:
					pointList.points.forEach(pointList.selected::add);
	...
	private class PointList extends GuiListSlot implements Comparator<PointEntry>
	{
		...
		{
			return !contents.isEmpty() && selected.contains(contents.get(index));
		}
		...
		private boolean filterMatch(PointEntry entry, String filter)
		{
			if (entry.isOreDict())
			{
