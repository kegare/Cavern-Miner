...
public class GuiSelectMob extends GuiScreen
{
	...
	public void drawScreen(int mouseX, int mouseY, float ticks)
	{
		...
		else if (!mobList.selected.isEmpty())
		{
			...
			if (detailInfo.isChecked() && selectedHoverChecker.checkHover(mouseX, mouseY))
			{
				...
				{
					values.add(getEntityName(new ResourceLocation(mob)));
				}
	...
	@Nullable
	protected String getEntityName(ResourceLocation key)
	{
		String entityName = EntityList.getTranslationName(key);

		if (!Strings.isNullOrEmpty(entityName))
		{
			return CavernMod.proxy.translate("entity." + entityName + ".name");
		}
	...
	protected class MobList extends GuiListSlot
	{
		...
		protected void drawSlot(int slot, int par2, int par3, int par4, int mouseX, int mouseY, float partialTicks)
		{
			...
			switch (nameType)
			{
				case 1:
					...
					break;
				default:
					name = GuiSelectMob.this.getEntityName(new ResourceLocation(entry));
					break;
			}
		...
		protected boolean filterMatch(String entry, String filter)
		{
			...
			String name = GuiSelectMob.this.getEntityName(new ResourceLocation(entry));
