...
public class WorldGenPervertedBirchTree extends WorldGenBirchTree
{
	private boolean useExtraRandomHeight;

	public WorldGenPervertedBirchTree(boolean notify, boolean useExtraRandomHeightIn)
	{
					...
					for (int y = pos.getY() - 3 + treeHeight; y <= pos.getY() + treeHeight; ++y)
					{
						...
						for (int x = pos.getX() - range; x <= pos.getX() + range; ++x)
						{
							...
							for (int z = pos.getZ() - range; z <= pos.getZ() + range; ++z)
							{
								...
								{
									IBlockState state2 = world.getBlockState(blockpos.setPos(x, y, z));
